...
local function createEmoteDropdown(FunTab, emotes)
    local emoteNames = {}
    local emoteMap = {}
    ...
    for _, emote in pairs(emotes) do
        ...
        emoteMap[emote.name] = emote.animationid
    end
    ...
    local Dropdown = FunTab:CreateDropdown({
        Name = "Emotes",
        Options = emoteNames,
        CurrentOption = {emoteNames[1] or " "}, -- Sélectionner la première option par défaut
        MultipleOptions = false,
        Flag = "emotes",
        Callback = function(Option)
            local AnimationId = emoteMap[Option]
            if AnimationId then
                -- Jouer l'animation pour le personnage local
                ...
                warn("Animation ID not found for option: " .. Option)
            end
        end,
    })
end

local function fetchEmotes(url)
    local success, response = pcall(function()
    ...
    if success then
        local successDecode, emoteData = pcall(function()
        ...
        if successDecode then
            ...
            warn("[AFEM] - Error decoding JSON: " .. tostring(emoteData))
        end
    else
        warn("[AFEM] - Error fetching data: " .. tostring(response))
    end

    -- Utilisation des données de secours en cas d'erreur
    return HttpService:JSONDecode('[{"id":14353423348,"animationid":"http://www.roblox.com/asset/?id=14352343065","name":"BabyQueen-BouncyTwirl"},{"id":14353421343,"animationid":"http://www.roblox.com/asset/?id=14352340648","name":"BabyQueen-FaceFrame"},{"id":16553249658,"animationid":"http://www.roblox.com/asset/?id=16553163212","name":"MaeStephens-PianoHands"}]')
end

-- Fonction principale pour initialiser le dropdown des emotes
local function initializeEmoteDropdown(FunTab)
    local emoteUrl = "https://yarhm.goteamst.com/static/emotes.json"
    local emotes = fetchEmotes(emoteUrl)
    createEmoteDropdown(FunTab, emotes)
end

return {
    initializeEmoteDropdown = initializeEmoteDropdown
}
